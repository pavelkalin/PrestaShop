...
class DbConnection
{
	...
	public function getContacts($email = null, $newsletter_guests = null)
	{
		...
		$sql = 'SELECT
			...
			LEFT JOIN
				'._DB_PREFIX_.'category_product cp ON (cp.id_product = od.product_id AND od.id_shop = '.$this->id_shop.')
			LEFT JOIN
				'._DB_PREFIX_.'category_lang cl ON (cl.id_category = cp.id_category AND cl.id_shop = '.$this->id_shop.' AND cl.id_lang = cu.id_lang)
			WHERE
					cu.newsletter = 1'.$where.'
				AND
					cu.id_shop = '.$this->id_shop.'
	...
	public function getCycleDay($api_key, $api_url)
	{
		...
		{
			$client = new JsonRpcClient($api_url);
			$results = $client->get_messages( $api_key,
				array('type'=>'autoresponder')
			);
	...
	public function updateApiSettings($apikey, $account_type, $crypto)
	{
		...
		return (bool)$this->db->autoExecute(_DB_PREFIX_.'getresponse_settings', $data, 'UPDATE', 'id_shop = '.$this->id_shop);
	}
	...
	public function updateWebformSettings($webform_id, $active_subscription, $sidebar, $style, $url)
	{
		...
		return (bool)$this->db->autoExecute(_DB_PREFIX_.'getresponse_webform', $data, 'UPDATE', 'id_shop = '.$this->id_shop);
	}
	...
	public function updateAutomationSettings($automation_to_edit, $campaign_id, $action, $cycle_day)
	{
		$data = array(
			'campaign_id' => pSQL($campaign_id),
			'action' => pSQL($action),
		...
		return (bool)$this->db->autoExecute(_DB_PREFIX_.'getresponse_automation', $data, 'UPDATE', 'id = '.$automation_to_edit);
	}
	...
	public function updateAutomationStatus($status, $id)
	{
		...
		return (bool)$this->db->autoExecute(_DB_PREFIX_.'getresponse_automation', $data, 'UPDATE', 'id_shop = '.$this->id_shop.' AND id = '.$id);
	}
	...
	{
		$data = array('category_id' => pSQL($category_id),
			'campaign_id' => pSQL($campaign_id),
			'action' => pSQL($action),
			'cycle_day' => pSQL($cycle_day),
			'id_shop' => $this->id_shop,
		...
		try {
			$this->db->autoExecute(_DB_PREFIX_.'getresponse_automation', $data, 'INSERT');
		} catch (Exception $e) {
			return false;
		}
	...
	{
		$sql = 'DELETE FROM `'._DB_PREFIX_.'getresponse_automation`
				WHERE `id` = '.(int)$automation_id;
	...
	public function exportSubscriber($api_key, $api_url, $campaign_id, $customers, $cycle_day)
	{
		...
		if (!empty($customers))
		{
			foreach ($customers as $customer)
			{
				...
				$r = $this->addContactToGR($api_key, $api_url, $campaign_id, $customer['firstname'],
					$customer['lastname'], $customer['email'], $customs, $cycle_day);
	...
	private function mapCustoms($customer, $customer_post, $type)
	{
		...
		if (!empty($custom_fields))
		{
			foreach ($custom_fields as $cf)
			{
				if ($type == 'export')
				{
					if (!empty($customer_post['custom_field']) && in_array($cf['custom_value'], array_keys($customer_post['custom_field'])))
	...
	public function addSubscriber($params, $apikey, $api_url, $campaign_id, $action, $cycle_day)
	{
		...
		if (!empty($action) && in_array($action, $allowed) == true && $action == 'create')
		{
			...
			if (isset($params[$prefix]->newsletter) && $params[$prefix]->newsletter == 1)
				$this->addContactToGR($apikey, $api_url, $campaign_id, $params[$prefix]->firstname,
					$params[$prefix]->lastname, $params[$prefix]->email, $customs, $cycle_day);
		}
		//update_contact
		else
		{
					...
					foreach ($automations as $autmation)
					{
						if (in_array($autmation['category_id'], $categories))
						{
							...
							if ($autmation['action'] == 'move')
								$this->moveContactToGr($apikey, $api_url, $campaign_id, $autmation['campaign_id'], $params[$prefix]->email);
							else if ($autmation['action'] == 'copy')
								$this->addContactToGR($apikey, $api_url, $campaign_id, $params[$prefix]->firstname,
									$params[$prefix]->lastname, $params[$prefix]->email, $customs, $cycle_day);
						}
						else
	...
	public function addContactToGR($api_key, $api_url, $campaign_id, $first_name = null, $last_name = null, $email, $customs, $cycle_day)
	{
		// required params
