<?php
/**
 * Written using the JSON RPC specification - http://json-rpc.org/wiki/specification
 *
...
class JsonRpcClient
{
    ...
    public function __call($method, $params)
    {
        ...
        $request = Tools::jsonEncode(array(
            'jsonrpc' => '2.0',
            'method'  => $method,
            'params'  => $params,
            'id'      => $request_id
        ...
        $validate_params = array
        (
            $response['id'] != $request_id => 'Request id: ' . $request_id .
                                              ' is different from Response id: ' . $response['id'],
    ...
    protected function debug($add, $show = false)
    {
        ...
        if (true === $show && ! empty( $debug )) {
            ...
            // performance summary
            $debug .= 'Request time: ' . round($end_time - $start_time, 3) . ' s Memory usage: ';
            $debug .= round(memory_get_usage() / 1024) . " kb\r\n";
            echo nl2br($debug);
