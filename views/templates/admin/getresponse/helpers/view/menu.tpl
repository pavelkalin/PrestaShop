...
<ul class="gr_menu">
	...
	<li class="{if $selected_tab == 'viawebform'}current-menu-item{/if}">
		{if $api_key}
			<a href="{$action_url|escape:'htmlall':'UTF-8'}&action=viawebform" class="gr-viawebform" title="viawebform">{l s='Subscription via a form' mod='getresponse'}</a>
		{else}
			<a class="inactive gr-viawebform" title="viawebform">{l s='Subscription via a form' mod='getresponse'}</a>
		{/if}
	</li>
