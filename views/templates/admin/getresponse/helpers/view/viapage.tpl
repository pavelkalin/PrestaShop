...
<form class="form-horizontal" action="{$action_url|escape:'htmlall':'UTF-8'}&amp;action=viapage" method="post">
	<div id="switch-box">
		<div class="control-group">
			...
			<div class="controls">
				<a title="Status" data-params="{ run: 'switch_viapage' }"  href="{$action_url|escape:'htmlall':'UTF-8'}&amp;action=viapage#iswitch" id="chckSubscriptionInput" class="switch {if $status == 'yes'}enabled{else}disabled{/if}">
				...
				<a class="gr-tooltip">
					<span class="gr-tip">
						<h5>{l s='Subscription' mod='getresponse'}</h5>
						<p>
							{l s='If you enable the switch, subscribers will be added immediately to your GetResponse account. You can stop this option anytime by switching the slider to OFF' mod='getresponse'}
						</p>
					</span>
				</a>
			</div>
	...
	<div id="form-box" {if $status == 'yes'} style="display:block"{else} style="display:none"{/if}>
		...
		<div class="control-group" id="campaignDiv">
			...
			<div class="controls">
				{if $campaigns}
					...
					<a href="#add_campaign" data-lightbox="{ selector: '#add_campaign' }" class="popup-link">{l s='Add new campaign' mod='getresponse'}</a>
				{else}
		...
		<div class="control-group" id="updateDiv">
			...
			<div class="controls">
				<label class="checkbox"  data-params="{ action: 'checkbox', run: 'switch_viapage_customs' }">
					...
					{l s='Checkout Update' mod='getresponse'}
				</label>
				<a class="gr-tooltip">
					<span class="gr-tip">
		...
		<div id="customNameFields" {if $update_address == 'yes'} style="display:block"{else} style="display:none"{/if}>
