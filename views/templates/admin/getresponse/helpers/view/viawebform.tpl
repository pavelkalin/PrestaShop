...
<form class="form-horizontal" action="{$action_url|escape:'htmlall':'UTF-8'}&action=viawebform" method="post">
	...
	<div id="form-box" {if $webform_status == 'yes'} style="display:block"{else} style="display:none"{/if}>
		<div class="control-group" id="webformIdDiv">
				...
				{if $webforms}
					<select name="webform_id" id="webformSidebar" class="gr_select fullselect">
						...
						{foreach $webforms as $wid => $webform}
							<option value="{$wid}" {if $webform_id == $wid}selected{/if}>{$webform['name']|escape:'htmlall':'UTF-8'} ({$campaigns[$webform['campaign']]|escape:'htmlall':'UTF-8'})</option>
						{/foreach}
					</select>
				{else}
					<h5>{l s='No webforms' mod='getresponse'}</h5>
				{/if}
			</div>
		</div>

		<div class="control-group" id="webformSidebarDiv">
		...
		<div class="control-group" id="webformStyleDiv">
