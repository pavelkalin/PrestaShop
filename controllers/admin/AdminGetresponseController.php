<?php
/**
 * This module integrate GetResponse and PrestaShop Allows subscribe via checkout page and export your contacts to GetResponse's campaign.
 *
...
class AdminGetresponseController extends ModuleAdminController
{
	...
	public function __construct()
	{
		...
		$this->gr_css_path		= _PS_MODULE_DIR_.'getresponse/css/';
	...
	public function setMedia()
	{
		...
		if (version_compare(_PS_VERSION_, '1.6') == '-1')
			$this->addCSS(array(_MODULE_DIR_.$this->module->name.'/css/gr-main-15.css'));
		else
			$this->addCSS(array(_MODULE_DIR_.$this->module->name.'/css/gr-main.css'));

		$this->addCSS(array(_MODULE_DIR_.$this->module->name.'/css/getresponse-custom-field.css'));
		$this->addJs(_MODULE_DIR_.$this->module->name.'/js/app.src-verified.js');
		$this->addJs(_MODULE_DIR_.$this->module->name.'/js/fullSelect.src-verified.async.js');
		$this->addJs(_MODULE_DIR_.$this->module->name.'/js/lightbox.src-verified.async.js');
		$this->addJs(_MODULE_DIR_.$this->module->name.'/js/growler.src-verified.async.js');
		$this->addJs(_MODULE_DIR_.$this->module->name.'/js/getresponse-custom-field.src-verified.js');
		$this->addJs(_MODULE_DIR_.$this->module->name.'/js/gr_main.js');

		// additional css styles for api view
		if (Tools::getValue('action') == 'api' || Tools::getValue('action') == '')
		{
			$this->addCSS(array(_MODULE_DIR_.$this->module->name.'/css/FormsValidateLc.css'));
			$this->addCSS(array(_MODULE_DIR_.$this->module->name.'/css/FormsEffectLc.css'));
			$this->addCSS(array(_MODULE_DIR_.$this->module->name.'/css/customcreate.css'));
			$this->addCSS(array(_MODULE_DIR_.$this->module->name.'/css/getresponse-pro.css'));
			$this->addCSS(array(_MODULE_DIR_.$this->module->name.'/css/helvetica-neue.css'));
		...
		if ($is_submit)
		{
			...
			if (!empty($api_key))
			{
				...
				if ($account_type[0] != 'gr' && empty($api_crypto))
					$this->context->smarty->assign(array('form_status' => 'error', 'status_text' => $this->l('For GetResponse360 Crypto can not be empty')));
				else
				{
					...
					if (!empty($campaigns))
					{
						...
						$this->context->smarty->assign(array('form_status' => 'success', 'status_text' => $this->l('API Key update successful')));
					}
					else
					{
						if ($account_type[0] != 'gr')
							$this->context->smarty->assign(array('form_status' => 'error', 'status_text' => $this->l('Wrong API key or Crypto')));
						else
				...
				$this->context->smarty->assign(array('form_status' => 'error', 'status_text' => $this->l('Api Key field can not be empty')));
		}
	...
	public function exportView()
	{
		...
		if ($is_submit)
		{
			...
			if (empty($campaign[0]) || $campaign[0] == '0')
				$this->context->smarty->assign(array('form_status' => 'error', 'status_text' => $this->l('No campaign selected')));
			else
			{
				...
				else
				{
					...
					if (empty($contacts))
						$this->context->smarty->assign(array('form_status' => 'error', 'status_text' => $this->l('No contacts to export')));
					else
					{
						$add_to_cycle	= Tools::getValue('add_to_cycle');
						$cycle_day = !empty($add_to_cycle) ? $cycle_day[0] : null;
						// export contacts to GR campaign
						$add = $this->db->exportSubscriber($this->apikey, $this->api_url, $campaign[0], $contacts, $cycle_day);

						// show notice
						if (is_array($add) && isset($add['status']) && $add['status'] == 1)
							$this->context->smarty->assign(array('form_status' => 'success', 'status_text' => $add['message']));
						else
							$this->context->smarty->assign(array('form_status' => 'error', 'status_text' => $add['message']));
					}
				}
			}
		}
		...
		if ($is_submit)
		{
			...
			if (!empty($campaign[0]) && $campaign[0] != '0' && !empty($status))
			{
				...
				else
				{
					...
					$this->context->smarty->assign(array('form_status' => 'success', 'status_text' => $this->l('Settings update successful')));
				}
			}
			else if (!empty($campaign[0]) && $campaign[0] == '0')
				$this->context->smarty->assign(array('form_status' => 'error', 'status_text' => $this->l('No campaign selected')));
		}
	...
	public function viawebformView()
	{
		...
		if ($is_submit)
		{
			...
			if (is_array($webform_id) && empty($webform_id[0]))
				$this->context->smarty->assign(array('form_status' => 'error', 'status_text' => $this->l('Web Form can not be empty')));
			else
			{
				// set web form info to DB
				$this->db->updateWebformSettings($webform_id[0], $webform_status, $webform_sidebar[0], $webform_style[0], $webforms[$webform_id[0]]['url']);
				$this->context->smarty->assign(array('form_status' => 'success', 'status_text' => $this->l('Settings update successful')));
			}
		}
	...
	public function automationView()
	{
		...
		if ($is_submit)
		{
			...
			if (empty($category[0]))
				$this->context->smarty->assign(array('form_status' => 'error', 'status_text' => $this->l('Category can not be empty')));
			else if (empty($campaign[0]))
				$this->context->smarty->assign(array('form_status' => 'error', 'status_text' => $this->l('Campaign can not be empty')));
			else if (empty($action[0]))
				$this->context->smarty->assign(array('form_status' => 'error', 'status_text' => $this->l('Action can not be empty')));
			else
			{
				$add_to_cycle	= Tools::getValue('add_to_cycle');
				$cycle_day = !empty($add_to_cycle) ? $cycle_day[0] : null;
				...
				$this->context->smarty->assign(array('form_status' => 'success', 'status_text' => $this->l('Automation created successful')));
			}
		}
		...
		if ($is_submit && $automation_to_edit)
		{
			...
			else
			{
				...
				$cycle_day = !empty($add_to_cycle) ? $cycle_day[0] : null;
				// set automation info to DB
				$this->db->updateAutomationSettings($automation_to_edit, $campaign[0], $action[0], $cycle_day);
				$this->context->smarty->assign(array('form_status' => 'success', 'status_text' => $this->l('Automation update successful')));
			}
		}
		...
		if ($delete_id)
		{
			...
			$this->context->smarty->assign(array('form_status' => 'success', 'status_text' => $this->l('Automation deleted successful')));
		}
		...
		if (is_array($customs))
		{
			foreach ($customs as $custom)
			{
				if (!empty($custom) && preg_match('/^[\w\-]+$/', $custom) == false)
					return array('form_status' => 'error', 'status_text' => 'Error - "'.$custom.'" '.$this->l('contains invalid characters'));
			}
		}
	...
	public function displayAjaxAddCampaign()
	{
		...
		// add new campaign to GR
		$add = $this->addCampaignToGR($campaign_name, $from_field, $reply_to_field);

		// show notice
		if (is_array($add) && isset($add['added']) && $add['added'] == 1)
			die(Tools::jsonEncode(array('type'=>'success', 'msg'=>'Campaign "'.$campaign_name.'" sucessfully created.', 'c'=>$campaign_name)));
		else
		{
			$error_message = 'Campaign "'.$campaign_name.'" has not been added ';

			if (preg_match('[Name already taken]', $add))
				die(Tools::jsonEncode(array('type' => 'error', 'msg' => $error_message.$this->l(' - Name already taken.'))));
			if (preg_match('[Total limit of campaigns exceeded]', $add))
				die(Tools::jsonEncode(array('type' => 'error', 'msg' => $error_message.$this->l(' - Total limit of campaigns exceeded.'))));
			if (preg_match('[Invalid email syntax]', $add))
				die(Tools::jsonEncode(array('type' => 'error', 'msg' => $error_message.$this->l(' - Invalid email syntax.'))));
			if (preg_match('[Missing From field]', $add))
				die(Tools::jsonEncode(array('type' => 'error', 'msg' => $error_message.$this->l(' - Missing From field.'))));
			if (preg_match('[Missing Reply-To field]', $add))
				die(Tools::jsonEncode(array('type' => 'error', 'msg' => $error_message.$this->l(' - Missing Reply-To field.'))));
			if (preg_match('[Missing confirmation subject]', $add))
				die(Tools::jsonEncode(array('type' => 'error', 'msg' => $error_message.$this->l(' - Missing confirmation subject'))));
			if (preg_match('[Missing confirmation body]', $add))
				die(Tools::jsonEncode(array('type' => 'error', 'msg' => $error_message.$this->l(' - Missing confirmation body.'))));
	 ...
	 * @return string
	 */
	private function addCampaignToGR($campaign_name, $from_field, $reply_to_field)
	{
		// required params
		if (empty($this->apikey) || empty($this->api_url))
			return false;
		...
		try
		{
			...
			$params = array (
				...
				'confirmation_subject'	=> 'e', // default
				'confirmation_body'		=> 'nc' // default
			);
